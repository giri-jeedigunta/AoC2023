...
function getPossibleGames(input, redCubes, greenCubes, blueCubes) {
  const games = input.trim().split('\n');

  const possibleGames = [];
  games.forEach(game => {
    const cubes = game
      .replace(/Game \d+: /, '') // Remove game ID
      .split(';')
      .map(set => set.trim().split(',').map(cube => cube.trim().split(' ')))
      .flat();

      console.log('cubes ---', cubes);

    const isValid = cubes.every(([count, color]) => {
      const numCubes = parseInt(count, 10);
      switch (color) {
        case 'red':
          return numCubes <= redCubes;
        case 'green':
          return numCubes <= greenCubes;
        case 'blue':
          return numCubes <= blueCubes;
        default:
          return false;
      }
    });
    ...
    if (isValid) {
      const gameId = parseInt(game.match(/Game (\d+)/)[1], 10);
...
const possibleGames = getPossibleGames(input, redCubes, greenCubes, blueCubes);
...
function getMinimumCubes(input) {
  const games = input.trim().split('\n');
  ...
  games.forEach(game => {
    ...
    const minCubes = cubes.reduce((min, [count, color]) => {
      const numCubes = parseInt(count, 10);
      switch (color) {
        case 'red':
          return { red: Math.max(min.red, numCubes), green: min.green, blue: min.blue };
        case 'green':
          return { red: min.red, green: Math.max(min.green, numCubes), blue: min.blue };
        case 'blue':
          return { red: min.red, green: min.green, blue: Math.max(min.blue, numCubes) };
        default:
          return min;
      }
...
const result = getMinimumCubes(input);
